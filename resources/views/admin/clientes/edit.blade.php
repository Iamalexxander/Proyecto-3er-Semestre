            ...
            <div class="card border-0 shadow-2xl" style="background: rgba(15, 23, 42, 0.95); backdrop-filter: blur(20px);">
                ...
                <div class="card-header border-bottom border-accent p-4"
                    ...
                    <div class="d-flex align-items-center">
                        ...
                        <div>
                            <h3 class="text-white mb-0">Editar Usuario</h3>
                            <p class="text-white-50 mb-0">Modificar usuario #{{ $usuario->id }}</p>
                        </div>
                    </div>
                </div>

                <div class="card-body p-4 text-white">
                    <form action="{{ route('admin.clientes.update', $usuario->id) }}" method="POST" class="needs-validation" id="editUserForm" novalidate>
                        @csrf
                        @method('PUT')
                            ...
                            <div class="col-md-6">
                                <div class="form-group">
                                    ...
                                    <select class="form-select form-select-lg bg-dark text-white border-accent @error('rol_id') is-invalid @enderror"
                                        ...
                                        <option value="1" {{ old('rol_id', $usuario->rol_id) == 1 ? 'selected' : '' }}>Admin</option>
                                        <option value="2" {{ old('rol_id', $usuario->rol_id) == 2 ? 'selected' : '' }}>Cliente</option>
                                    </select>
                                    @error('rol_id')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    form.addEventListener('submit', function(event) {
        ...
        // No validamos password ya que es opcional en la edición
